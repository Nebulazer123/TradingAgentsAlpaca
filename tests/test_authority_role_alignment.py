...
def test_execution_requires_bounded_paper_and_separate_normal_trade_inputs():
    ...
    board_constraint = _registry()["automation_constraints"][
    ...
    assert board_constraint["allowed_actions"] == ["trade_decision"]
    assert board_constraint["required_outputs"] == ["portfolio_decision"]
    assert set(board_constraint["forbidden_effects"]) == {
        "risk_change",
        "strategy_change",
        "promotion_change",
        "freeze",
...
_LIVE_WRITE_CALLER_CLASSIFICATIONS = {
        ...
        ("cli/main.py", "alpaca_reconcile_observer", "_alpaca_live_client"): "read-only",
        (
...
_HTTP_MUTATION_CLASSIFICATIONS: dict[tuple[str, int, str, str], str] = {
    ...
    ("cli/main.py", 6558, "_overnight_ticker_process_main", "raw-http-put"): (
        "non-trading-local-process-result-queue"
    ),
    ("cli/main.py", 6570, "_overnight_ticker_process_main", "raw-http-put"): (
