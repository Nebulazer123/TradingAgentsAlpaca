...
class DataflowsConfigIsolationTests(unittest.TestCase):
    ...
    def test_get_config_returns_deep_copy(self):
        ...
        fresh = get_config()
        self.assertEqual(fresh["data_vendors"]["core_stock_apis"], "yfinance,tiingo,massive,alpha_vantage")
    ...
    def test_partial_nested_update_preserves_existing_defaults(self):
        ...
        self.assertEqual(fresh["data_vendors"]["fundamental_data"], "fmp,eodhd,finnhub,sec,yfinance,alpha_vantage")
        self.assertEqual(
            fresh["data_vendors"]["news_data"],
            "marketaux,finnhub,newsapi,google_news,eodhd,tiingo,fmp,alpaca_news,yfinance,alpha_vantage",
        )
        self.assertEqual(fresh["data_vendors"]["macro_context"], "fred,bls,bea,eia,treasury_fiscal")
        self.assertEqual(fresh["data_vendors"]["sentiment_data"], "stocktwits,reddit,eodhd")
    ...
    def test_empty_nested_dict_update_clears_previous_values(self):
        set_config({"tool_vendors": {"get_stock_data": "alpha_vantage"}})
        set_config({"tool_vendors": {}})

        fresh = get_config()
        self.assertEqual(fresh["tool_vendors"], {})
