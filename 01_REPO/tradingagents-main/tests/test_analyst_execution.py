...
class AnalystExecutionPlanTests(unittest.TestCase):
    def test_build_plan_preserves_selected_order(self):
        ...
        self.assertEqual([[spec.key for spec in batch] for batch in plan.batches], [["news", "market"]])
    ...
    def test_build_plan_batches_by_concurrency_limit(self):
        ...
        self.assertEqual(
            [[spec.key for spec in batch] for batch in plan.batches],
            [["market", "social"], ["news", "fundamentals"]],
        )
