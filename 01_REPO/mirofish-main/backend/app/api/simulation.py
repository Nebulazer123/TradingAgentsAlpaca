...
def _parse_bool(value, default: bool = False) -> bool:
    """Parse JSON/form-ish booleans without treating 'false' as truthy."""
    if value is None:
        return default
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - failed/paused: 运行失败或暂停，但准备文件仍可复用
        ...
        if status in prepared_statuses and config_generated:
            ...
            # 如果状态是preparing/paused但文件已完成，自动更新状态为ready
            if status in {"preparing", "paused"}:
                try:
                    ...
                    logger.info(f"自动更新模拟状态: {simulation_id} {status} -> ready")
                    status = "ready"
                except Exception as e:
...
def start_simulation():
    ...
    请求（JSON）：
        {
            ...
            "enable_graph_memory_update": true,    // 可选: 是否将Agent活动动态更新到Zep图谱记忆
            "graph_id": "mirofish_xxxx",           // 可选: 启用图谱记忆时显式指定图谱
    ...
    try:
        ...
        requested_graph_memory_update = data.get('enable_graph_memory_update')  # 可选：是否启用图谱记忆更新
        requested_graph_id = data.get('graph_id')  # 可选：显式图谱ID
        ...
        project_graph_id = project.graph_id if project else None
        known_graph_ids = {item for item in (state_graph_id, project_graph_id) if item}
        if requested_graph_id and known_graph_ids and requested_graph_id not in known_graph_ids:
            return jsonify({
                "success": False,
                "error": f"graph_id does not match simulation/project graph: requested={requested_graph_id}"
            }), 400
        graph_id = requested_graph_id or state_graph_id or project_graph_id
        enable_graph_memory_update = _parse_bool(
            requested_graph_memory_update,
            default=bool(graph_id),
        )
        ...
        response_data['max_rounds_applied'] = run_state.total_rounds
        response_data['graph_memory_update_enabled'] = run_state.graph_memory_update_enabled
        response_data['force_restarted'] = force_restarted
        if run_state.graph_memory_update_enabled:
            response_data['graph_id'] = run_state.graph_id
...
def get_env_status():
    ...
    try:
        ...
        env_alive = bool(readiness.get("live_interviews_available"))
        ...
        return jsonify({
            ...
            "data": {
                ...
                "live_interviews_available": readiness.get("live_interviews_available", False),
                "runner_process_alive": readiness.get("runner_process_alive", False),
                "ipc_env_alive": readiness.get("ipc_env_alive", False),
                "stale_env_status": readiness.get("stale_env_status", False),
                "runner_status": readiness.get("runner_status"),
                "process_pid": readiness.get("process_pid"),
                "env_status": readiness.get("env_status"),
                "env_status_timestamp": readiness.get("env_status_timestamp"),
