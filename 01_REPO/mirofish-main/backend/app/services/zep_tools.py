...
class SearchResult:
    ...
    def to_text(self) -> str:
        ...
        if self.provenance:
            text_parts.append(f"Source provenance: {json.dumps(self.provenance, ensure_ascii=False)}")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.provenance.get("graph_wide_fetch_deferred"):
            text_parts.append("- Graph-wide node/edge panorama: deferred in interactive Step 4 safe mode")
            text_parts.append(f"- Source provenance: {json.dumps(self.provenance, ensure_ascii=False)}")
        else:
            text_parts.extend([
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        if self.deferred or self.mode != "live":
            text_parts = [
                "## Replay Evidence / Stage 5 Interview Targets",
                f"**Topic:** {self.interview_topic}",
                f"**Mode:** {self.mode}",
                "**Live interviews:** deferred because this report is being generated from a stopped/completed simulation run.",
                "\n### Target Selection",
                self.selection_reasoning or "Selected from Stage 3 telemetry and profile evidence.",
                "\n### Proposed Questions",
            ]
            for i, question in enumerate(self.interview_questions, 1):
                text_parts.append(f"{i}. {question}")
            if self.selected_agents:
                text_parts.append("\n### Candidate Stage 5 Targets")
                for i, agent in enumerate(self.selected_agents, 1):
                    agent_name = agent.get("realname") or agent.get("username") or agent.get("name") or f"Agent {agent.get('agent_id', i)}"
                    role = agent.get("profession") or agent.get("role") or agent.get("description") or "unknown role"
                    text_parts.append(f"{i}. {agent_name} - {role}")
            text_parts.append("\n### Summary")
            text_parts.append(self.summary or "Live interview transcripts were not produced in Step 4 report mode.")
            if self.provenance:
                text_parts.append(f"\nSource provenance: {json.dumps(self.provenance, ensure_ascii=False)}")
            return "\n".join(text_parts)
...
class ZepToolsService:
    ...
    MAX_RETRIES = 3
    RETRY_DELAY = 2.0
    MAX_GRAPH_QUERY_CHARS = 380
    
    def __init__(
        self,
        api_key: Optional[str] = None,
        llm_client: Optional[LLMClient] = None,
        report_cache: Optional[ZepReportCache] = None,
    ...
    def _call_with_retry(self, func, operation_name: str, max_retries: int = None):
        ...
        now = time.time()
        if now < getattr(self, "_rate_limited_until", 0.0):
            retry_in = int(self._rate_limited_until - now)
            raise RuntimeError(f"Zep rate-limit cooldown active for {retry_in}s")
            ...
            try:
                ...
                if self._is_rate_limit_error(e):
                    ...
                    self._rate_limited_until = max(
                        getattr(self, "_rate_limited_until", 0.0),
                        time.time() + retry_after,
                    )
                    logger.warning(
                        "Zep rate limit hit during %s; cooling down for %ss",
                        operation_name,
                        retry_after,
                    )
                    break
    ...
    @classmethod
    def _normalize_graph_query(cls, query: Any) -> str:
        """Normalize Zep graph queries to stay under the cloud API 400-char limit."""
        text = re.sub(r"\s+", " ", str(query or "")).strip()
        if not text:
            return "MiroFish simulation context"
        if len(text) <= cls.MAX_GRAPH_QUERY_CHARS:
            return text

        shortened = text[:cls.MAX_GRAPH_QUERY_CHARS].rstrip()
        logger.warning(
            "Zep graph query truncated from %s to %s characters for API limit",
            len(text),
            len(shortened),
        )
        return shortened
    ...
    def _extract_retry_after(error: Exception) -> Optional[int]:
        match = re.search(r"retry-after['\"]?\s*[:=]\s*['\"]?(\d+)", str(error), re.IGNORECASE)
        if not match:
            return None
        try:
    ...
    @staticmethod
    def _empty_search_result(query: str) -> SearchResult:
        return SearchResult(
            facts=[],
            edges=[],
            nodes=[],
    ...
    def _zep_live_graph_search_payload(
        self,
        graph_id: str,
        query: str,
        limit: int,
        ...
        """Call Zep graph.search once and return plain dictionaries."""
    ...
    def search_graph(
        ...
        Returns:
            SearchResult: 搜索结果
        """
        normalized_query = normalize_query(query) if safe_mode or self.report_cache else self._normalize_graph_query(query)
        logger.info(t("console.graphSearch", graphId=graph_id, query=normalized_query[:50]))

        if self.report_cache:
            outcome = self.report_cache.search(
                tool_name=tool_name,
                query=normalized_query,
                limit=limit,
                scope=scope,
                section_index=section_index,
                live_search=lambda q, l, s: self._zep_live_graph_search_payload(graph_id, q, l, s),
            )
            return SearchResult(
                facts=outcome.facts,
                edges=outcome.edges,
        ...
        try:
            ...
            return SearchResult(
                ...
                provenance={"source": PROVENANCE_LIVE_ZEP},
            )
            ...
            if self._is_rate_limit_error(e):
                logger.warning(
                    "Zep graph search rate-limited; skipping all-graph local fallback for query=%s",
                    normalized_query[:80],
                )
                result = self._empty_search_result(normalized_query)
                result.provenance = {"source": PROVENANCE_RATE_LIMITED, "error": str(e)[:500]}
                return result
            if safe_mode:
                result = self._empty_search_result(normalized_query)
                result.provenance = {"source": PROVENANCE_PENDING, "skipped_reason": "safe_mode_no_graph_wide_fallback", "error": str(e)[:500]}
                return result
            # 降级：使用本地关键词匹配搜索
    ...
    def get_graph_statistics(self, graph_id: str, safe_mode: bool = False) -> Dict[str, Any]:
        """
        获取图谱的统计信息
        ...
        if safe_mode:
            if self.report_cache:
                self.report_cache.diagnostics.skipped_graph_wide_calls += 2
            return {
                "graph_id": graph_id,
                "total_nodes": None,
                "total_edges": None,
                ...
                "provenance": {
                    "source": PROVENANCE_PENDING,
                    "reason": "interactive_step4_safe_mode_defers_all_node_all_edge_statistics",
                },
            }
        ...
        try:
            ...
            logger.warning("Graph statistics degraded: failed to fetch nodes for %s: %s", graph_id, e)
            nodes = []

        try:
            edges = self.get_all_edges(graph_id)
        except Exception as e:
            logger.warning("Graph statistics degraded: failed to fetch edges for %s: %s", graph_id, e)
            edges = []
    ...
    def get_simulation_context(
        ...
        try:
            query = (
                "MiroFish Step 4 simulation context macro broker options AI bot "
                "institutional liquidity TradingAgents validation"
                if safe_mode
                else simulation_requirement
            )
            search_result = self.search_graph(
                graph_id=graph_id,
                query=query,
            ...
            logger.warning("Simulation context degraded: search failed for %s: %s", graph_id, e)
            warnings.append(f"graph search failed: {str(e)[:160]}")
            search_result = self._empty_search_result(normalize_query(simulation_requirement) if safe_mode else self._normalize_graph_query(simulation_requirement))
        
        # 获取图谱统计
        try:
            stats = self.get_graph_statistics(graph_id, safe_mode=safe_mode)
        except Exception as e:
            logger.warning("Simulation context degraded: graph statistics failed for %s: %s", graph_id, e)
            warnings.append(f"graph statistics failed: {str(e)[:160]}")
            stats = {
                "graph_id": graph_id,
                "total_nodes": 0,
                "total_edges": 0,
        ...
        if safe_mode:
            if self.report_cache:
                self.report_cache.diagnostics.skipped_graph_wide_calls += 1
            warnings.append("entity node fetch deferred in interactive Step 4 safe mode")
            all_nodes = []
        else:
            try:
                all_nodes = self.get_all_nodes(graph_id)
            except Exception as e:
                logger.warning("Simulation context degraded: entity node fetch failed for %s: %s", graph_id, e)
                warnings.append(f"entity node fetch failed: {str(e)[:160]}")
                all_nodes = []
    ...
    def insight_forge(
        ...
        if safe_mode:
            sub_queries = [
                ...
                normalize_query(f"{query} causal attribution macro broker options AI bot institutional liquidity"),
            ][: max(1, min(max_sub_queries, 2))]
        else:
        ...
        max_entity_details = 8
        if len(entity_uuids) > max_entity_details:
            logger.info(
                "InsightForge entity detail fetch capped: %s -> %s",
                len(entity_uuids),
                max_entity_details,
            )

        for uuid in ([] if safe_mode else list(entity_uuids)[:max_entity_details]):
            if not uuid:
                continue
            if time.time() < getattr(self, "_rate_limited_until", 0.0):
                logger.warning("Skipping remaining entity details during Zep rate-limit cooldown")
                break
            try:
                # 单独获取每个相关节点的信息
...
返回JSON格式的子问题列表。"""
    ...
    def panorama_search(
        ...
        if safe_mode:
            ...
            result.active_count = len(search_result.facts)
            result.historical_facts = []
            result.historical_count = 0
            result.provenance = {
                "graph_wide_fetch_deferred": True,
                "reason": "interactive_step4_safe_mode_uses_capped_graph_search_only",
                **(search_result.provenance or {}),
            }
            logger.info(
                "PanoramaSearch safe mode complete via graph.search only: facts=%s source=%s",
                result.active_count,
        ...
        try:
            ...
            logger.warning("PanoramaSearch degraded: failed to fetch nodes for %s: %s", graph_id, e)
            all_nodes = []
        ...
        try:
            ...
            logger.warning("PanoramaSearch degraded: failed to fetch edges for %s: %s", graph_id, e)
            all_edges = []
    ...
    def interview_agents(
        ...
        active_report_mode = report_mode or self.report_mode
        if active_report_mode in {"zep_throttle_cached", "step4_report", "safe_step4"} and self._is_simulation_stopped_or_completed(simulation_id):
            targets = self._offline_interview_targets(simulation_id, max_agents=max_agents)
            questions = custom_questions or self._default_replay_interview_questions(interview_requirement)
            result.mode = "replay_deferred"
            result.deferred = True
            result.total_agents = len(targets)
            result.selected_agents = targets[:max_agents]
            result.interview_questions = questions
            result.selection_reasoning = (
                "Live interviews are deferred for completed/stopped Stage 3 runs. "
                "These are Stage 5 target agents selected from local profiles and telemetry evidence."
            )
            result.summary = (
                "No live interview transcript was generated in Step 4. Use these replay-evidence targets "
                "for Stage 5 interviews if a live interview environment is started later."
            )
            result.provenance = {
                "source": PROVENANCE_LOCAL_TELEMETRY,
                "simulation_id": simulation_id,
                "report_mode": active_report_mode,
                "live_interviews_deferred": True,
            }
            return result
    ...
    def _simulation_dir(self, simulation_id: str) -> str:
        return os.path.abspath(os.path.join(Config.UPLOAD_FOLDER, "simulations", simulation_id))

    def _is_simulation_stopped_or_completed(self, simulation_id: str) -> bool:
        """Best-effort local check used to keep Step 4 from calling live interviews."""
        sim_dir = self._simulation_dir(simulation_id)
        status_files = [
            os.path.join(sim_dir, "run_state.json"),
            os.path.join(sim_dir, "simulation_state.json"),
            os.path.join(sim_dir, "status.json"),
        ]
        stopped_values = {"stopped", "completed", "complete", "finished", "failed", "cancelled"}
        for path in status_files:
            ...
            statuses = [
                data.get("status"),
                data.get("state"),
                data.get("runner_status"),
                data.get("simulation_status"),
            ]
            if any(str(value or "").lower() in stopped_values for value in statuses):
                return True
            if data.get("twitter_completed") is True and data.get("reddit_completed") is True:
                return True
        return False

    def _default_replay_interview_questions(self, topic: str) -> List[str]:
        return [
            f"What evidence from your simulated actions best explains your view on {topic}?",
            "Which macro, broker, options, AI-bot, or liquidity factors most changed your behavior?",
            "What would need to be validated against real market data before TradingAgents should act on this signal?",
        ]

    def _offline_interview_targets(self, simulation_id: str, max_agents: int = 5) -> List[Dict[str, Any]]:
        profiles = self._load_agent_profiles(simulation_id)
        if not profiles:
            return []
        ...
        candidate_files = [
            os.path.join(sim_dir, "stage3_telemetry.json"),
            os.path.join(sim_dir, "telemetry_summary.json"),
            os.path.join(sim_dir, "postrun_telemetry.json"),
        ]
        for name in os.listdir(sim_dir) if os.path.isdir(sim_dir) else []:
            if name.endswith(".json") and "telemetry" in name.lower():
                candidate_files.append(os.path.join(sim_dir, name))
        ...
        for path in candidate_files:
            ...
            for key in (
                ...
                "stage05_interview_targets",
                "suggested_stage05_interview_targets",
                "top_influential_agents",
            ):
                raw_targets = data.get(key)
                if not isinstance(raw_targets, list):
                    continue
                for item in raw_targets:
                    if isinstance(item, dict):
                        value = item.get("agent_id") or item.get("id") or item.get("agent_index") or item.get("username")
                    else:
                        value = item
                    if value is not None and str(value) not in preferred_ids:
                        preferred_ids.append(str(value))
                if preferred_ids:
                    break
            if preferred_ids:
                break

        indexed_profiles: List[Dict[str, Any]] = []
        for idx, profile in enumerate(profiles):
            if not isinstance(profile, dict):
                continue
            enriched = dict(profile)
            enriched.setdefault("agent_id", enriched.get("id", idx))
            indexed_profiles.append(enriched)
        ...
        if preferred_ids:
            for wanted in preferred_ids:
                for profile in indexed_profiles:
                    identifiers = {
                        str(profile.get("agent_id")),
                        str(profile.get("id")),
                        str(profile.get("username")),
                        str(profile.get("realname")),
                    }
                    if wanted in identifiers and profile not in selected:
