...
def _remove_file_best_effort(path: str, *, retries: int = 5, delay: float = 0.05) -> None:
    """Remove an IPC file, tolerating short Windows file-handle races."""
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                logger.warning(f"清理IPC文件失败: {path}, {exc}")
                return
            time.sleep(delay)
        ...
        while time.time() - start_time < timeout:
            if os.path.exists(response_file):
                try:
                    ...
                    # 清理命令和响应文件
                    _remove_file_best_effort(command_file)
                    _remove_file_best_effort(response_file)
