...
ECONOMIC_VALIDATION_RESULT_SCHEMA = "economic_validation_result/v1"
...
def build_validation_evaluation_result(
    ...
    *,
    ...
    """Build a complete, arm-by-arm result for the sealed validation partition."""
...
def validate_economic_validation_result(value: object) -> EconomicValidationResult:
    ...
    rebuilt_metrics = _canonical_arm_metrics(
        arm_input,
        validation_event_count=len(event_ids),
    )
    rebuilt = _build_result_from_components(
        protocol_id=values["protocol_id"],  # type: ignore[arg-type]
