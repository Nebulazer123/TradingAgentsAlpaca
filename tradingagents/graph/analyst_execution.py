...
class AnalystExecutionPlan:
    ...
    batches: list[list[AnalystNodeSpec]]
...
ANALYST_NODE_SPECS: dict[str, AnalystNodeSpec] = {
    ...
    "social": AnalystNodeSpec(
        ...
        # Sentiment is a prefetch-only analyst in this repo. The read-only
        # sentiment context tool remains available, but the graph should not
        # register a dead tool round that this analyst never emits.
        tool_node=None,
...
def build_analyst_execution_plan(
    ...
    batches = [
        specs[i : i + concurrency_limit]
        for i in range(0, len(specs), concurrency_limit)
    ]
