...
Read-only active coverage estimator:

`C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\mirofish_active_coverage_estimator.py`

Post-run telemetry extractor:

`C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\mirofish_postrun_telemetry.py`

Predictive-quality artifacts:

- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_PRE_RUN_ACCURACY_AUDIT.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_EVENT_BEAT_MAP.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_STATE_VARIABLES.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_CAUSAL_ATTRIBUTION_PLAN.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_OPTIONS_MICROSTRUCTURE_BRIEF.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_JUNE4_LIVE_CONTEXT_PATCH.md`
- `C:\Users\Corbin\Documents\Coding projects\mirofish-main\docs\mirror_fish\MIRROR_FISH_STAGE05_INTERVIEW_PLAN.md`
...
8. Review the predictive-quality audit: 30 scheduled event beats, 7 forecast-ballot rounds, 14 state variables, 525 broker/account-segmented actors, and expected mid dual-platform actions around `1257`.
9. Review the actual prepared-config cost estimate.
10. Confirm output paths are known.
11. Stop and ask for explicit user approval before the real stage 03 simulation start.
...
Read-only active coverage command:

```powershell
python docs\mirror_fish\mirofish_active_coverage_estimator.py --config backend\uploads\simulations\sim_974459649906\simulation_config.json --max-rounds 30
```
...
The readiness bundle collects env preview, workflow audit, actor-population audit, predictive-quality evidence, active-coverage estimate, preflight, actual prepared-config cost, backend refresh proof, completion-audit status, objective-alignment status, an approval-gate ledger, closed real-stage-03 gate, and draft-only handoff status. The completion audit must say `safe_to_mark_goal_complete=false` until the real stage 03 run, report, deep interactions, synthesis, and final handoff are complete. The workflow audit verifies local source coverage for all five creator stages. It also documents that "automatic demand analysis" is represented by automated requirement analysis, generated time/event/agent configs, hot topics, narrative direction, and ReportAgent tools rather than a single endpoint named demand analysis.
