  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div class="graph-mode-badge" :class="{ warning: graphModeIsPreview }">
          <strong>{{ graphModeTitle }}</strong>
          <span>{{ graphModeDetail }}</span>
        </div>
...
const graphModeIsPreview = computed(() => {
  return props.graphData?.preview_mode && props.graphData.preview_mode !== 'zep_full_graph'
})

const graphModeTitle = computed(() => {
  const mode = props.graphData?.preview_mode
  if (mode === 'zep_full_graph') return 'Zep full graph'
  if (mode === 'report_graph_search') return 'Zep search preview'
  if (mode === 'report_sections') return 'Report preview only'
  return 'Graph data'
})

const graphModeDetail = computed(() => {
  const nodes = props.graphData?.node_count ?? props.graphData?.nodes?.length ?? 0
  const edges = props.graphData?.edge_count ?? props.graphData?.edges?.length ?? 0
  const limits = props.graphData?.display_limits
  const truncated = props.graphData?.possibly_truncated
  const capText = limits
    ? ` display cap ${limits.max_nodes} nodes / ${limits.max_edges} edges`
    : ''
  const truncatedText = truncated?.nodes || truncated?.edges ? ' possibly truncated' : ''
  return `${nodes} nodes / ${edges} edges${capText}${truncatedText}`
})
...
.graph-mode-badge.warning {
  border-color: #F3D08A;
  background: rgba(255, 250, 235, 0.96);
  color: #7A4C00;
}

.graph-mode-badge strong {
  font-size: 12px;
  line-height: 1.2;
}

.graph-mode-badge span {
  font-size: 11px;
  line-height: 1.25;
  color: inherit;
  opacity: 0.85;
