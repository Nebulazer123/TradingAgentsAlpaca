  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.runner_status === 'failed') {
        addLog(data.error ? `Simulation failed: ${data.error}` : 'Simulation failed')
        phase.value = 0
        stopPolling()
        emit('update-status', 'error')
        return
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.locked || res.data.status === 'report_in_progress') {
        addLog(t('log.reportRequestSent'))
      } else {
...
onMounted(() => {
  ...
  if (props.simulationId) {
    if (props.autoStart) {
      ...
      addLog('Passive monitor mode: start disabled; polling existing run status.')
      phase.value = 1
      emit('update-status', 'processing')
      fetchRunStatus()
