...
from ..utils.round_event_context import (
    max_rounds_source,
...
class SimulationRunState:
    ...
    max_rounds_source: str = "time_config"
    graph_memory_update_enabled: bool = False
    graph_id: Optional[str] = None
    launch_command: List[str] = field(default_factory=list)
    preserve_interview_window: bool = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "preserve_interview_window": self.preserve_interview_window,
        }
...
class SimulationRunner:
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            state = SimulationRunState(
                ...
                max_rounds_source=data.get("max_rounds_source", "time_config"),
                graph_memory_update_enabled=data.get("graph_memory_update_enabled", False),
                graph_id=data.get("graph_id"),
                launch_command=data.get("launch_command", []),
                preserve_interview_window=data.get("preserve_interview_window", True),
            )
    ...
    def start_simulation(
        ...
        effective_max_rounds_source = max_rounds_source(config, max_rounds)
        ...
        # 如果指定了最大轮数，或配置了 scheduled_events，则截断
        if effective_max_rounds is not None and effective_max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                logger.info(
                    "轮数已截断: "
                    f"{original_rounds} -> {total_rounds} "
                    f"(max_rounds={effective_max_rounds}, source={effective_max_rounds_source})"
                )
        ...
        state = SimulationRunState(
            ...
            max_rounds_applied=total_rounds,
            max_rounds_source=effective_max_rounds_source,
            graph_memory_update_enabled=enable_graph_memory_update,
            graph_id=graph_id if enable_graph_memory_update else None,
            preserve_interview_window=True,
        )
        ...
        try:
            ...
            # 如果指定了最大轮数，添加到命令行参数
            if effective_max_rounds is not None and effective_max_rounds > 0:
                cmd.extend(["--max-rounds", str(effective_max_rounds)])
            state.launch_command = cmd.copy()
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        state.completed_at = datetime.now().isoformat()
        cls._save_run_state(state)
        cls._mark_env_status(simulation_id, "stopped")
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        """
        return bool(cls.get_live_interview_readiness(simulation_id).get("live_interviews_available"))

    @classmethod
    def _mark_env_status(cls, simulation_id: str, status: str):
        """Best-effort env_status writer used when backend cleanup kills a runner."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        os.makedirs(sim_dir, exist_ok=True)
        ...
        try:
            ...
            logger.warning(f"更新环境状态失败: simulation={simulation_id}, status={status}, error={e}")

    @classmethod
    def _has_live_runner_process(cls, simulation_id: str, state: Optional[SimulationRunState] = None) -> bool:
        """Return True only when the runner process is still available for IPC commands."""
        process = cls._processes.get(simulation_id)
        if process and process.poll() is None:
            return True
    ...
    @classmethod
    def get_live_interview_readiness(cls, simulation_id: str) -> Dict[str, Any]:
        """Return authoritative live Step 5 readiness.

        A stale ``env_status.json`` alone is not enough. True live interviews
        require a live runner process that can receive IPC plus an alive IPC
        environment marker from the simulation script.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        state = cls.get_run_state(simulation_id)
        runner_status = state.runner_status.value if state else "unknown"
        process_pid = state.process_pid if state else None
        sim_dir_exists = os.path.exists(sim_dir)

        runner_process_alive = False
        if sim_dir_exists:
            runner_process_alive = cls._has_live_runner_process(simulation_id, state)

        ipc_env_alive = False
        env_status_detail = {
            "status": "missing" if not sim_dir_exists else "stopped",
            "twitter_available": False,
            "reddit_available": False,
            "timestamp": None,
        }
        if sim_dir_exists:
            env_status_detail = cls.get_env_status_detail(simulation_id)
            try:
                ipc_env_alive = SimulationIPCClient(sim_dir).check_env_alive()
            except Exception:
                ipc_env_alive = False

        stale_env_status = bool(ipc_env_alive and not runner_process_alive)
        status_allows_interview = runner_status in {
            RunnerStatus.RUNNING.value,
            RunnerStatus.COMPLETED.value,
        }
        live_available = bool(
            sim_dir_exists
            and runner_process_alive
            and ipc_env_alive
            and status_allows_interview
        )

        if not sim_dir_exists:
            reason = "simulation directory missing"
        elif not runner_process_alive and ipc_env_alive:
            reason = "stale env_status says alive but runner process is not available"
        elif not runner_process_alive:
            reason = "runner process is not available"
        elif not ipc_env_alive:
            reason = "IPC environment is not alive"
        elif not status_allows_interview:
            reason = f"runner status does not allow live interviews: {runner_status}"
        else:
            reason = "live interview IPC is available"

        return {
            "simulation_id": simulation_id,
            "live_interviews_available": live_available,
            "runner_process_alive": runner_process_alive,
            "ipc_env_alive": ipc_env_alive,
            "stale_env_status": stale_env_status,
            "runner_status": runner_status,
            "process_pid": process_pid,
            "env_status": env_status_detail.get("status"),
            "twitter_available": env_status_detail.get("twitter_available", False),
            "reddit_available": env_status_detail.get("reddit_available", False),
            "env_status_timestamp": env_status_detail.get("timestamp"),
            "reason": reason,
        }
    ...
    def interview_agent(
        ...
        readiness = cls.get_live_interview_readiness(simulation_id)
        if not readiness.get("live_interviews_available"):
            raise ValueError(
                "模拟环境未处于真实可采访状态，无法执行Interview: "
                f"{simulation_id}; reason={readiness.get('reason')}; "
                f"runner_status={readiness.get('runner_status')}; "
                f"process_pid={readiness.get('process_pid')}; "
                f"runner_process_alive={readiness.get('runner_process_alive')}; "
                f"ipc_env_alive={readiness.get('ipc_env_alive')}"
            )
