...
class GraphBuilderService:
    ...
    def get_graph_data(
        ...
        Args:
            graph_id: 图谱ID
            max_nodes: 最多返回的节点数，避免浏览器渲染过载
            max_edges: 最多返回的边数，避免浏览器渲染过载
            page_size: Zep分页大小
            
        Returns:
            包含nodes和edges的字典，包括时间信息、属性等详细数据
        """
        nodes = fetch_all_nodes(self.client, graph_id, page_size=page_size, max_items=max_nodes)
        edges = fetch_all_edges(self.client, graph_id, page_size=page_size, max_items=max_edges)
        ...
        return {
            ...
            "possibly_truncated": {
                "nodes": len(nodes_data) >= max_nodes,
                "edges": len(edges_data) >= max_edges,
            },
            "preview_mode": "zep_full_graph",
        }
