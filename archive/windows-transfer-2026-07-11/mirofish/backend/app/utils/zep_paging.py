...
def _retry_after_seconds(error: Exception) -> float | None:
    ...
    reset_match = re.search(r"x-ratelimit-reset['\"]?\s*[:=]\s*['\"]?(\d+(?:\.\d+)?)", text, re.IGNORECASE)
    if reset_match:
        reset_at = float(reset_match.group(1))
        wait_for = reset_at - time.time()
        if wait_for > 0:
            return wait_for
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    transient_errors = (ConnectionError, TimeoutError, OSError, InternalServerError)

    for attempt in range(max_retries):
        try:
            return api_call(*args, **kwargs)
        except Exception as e:
            if not isinstance(e, transient_errors) and not _is_rate_limit_error(e):
                raise
            ...
            if attempt < max_retries - 1:
                ...
                if _is_rate_limit_error(e):
                    logger.warning(
                        f"Zep {page_description} rate-limited, retrying in {sleep_for:.1f}s..."
                    )
                else:
                    logger.warning(
...
def fetch_all_nodes(
    ...
    """分页获取图谱节点，最多返回 max_items 条（默认 10000）。每页请求自带重试。"""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """分页获取图谱所有边，最多返回 max_items 条（默认 20000）。每页请求自带重试。"""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
